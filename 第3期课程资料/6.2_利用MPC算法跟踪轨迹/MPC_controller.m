function    [Delta_real,v_real,targetIdx,latError,U ] = ...
    MPC_controller(targetIdx,currentPos,currentHeading,refPos,refHeading,refDelta,dt,L,U,refSpeed)
%% MPC预设参数
Nx = 3;         % 状态量的个数
Nu = 2;         % 控制量的个数
Np = 60;        % 预测步长
...
%% 原运动学误差状态空间方程的相关矩阵
% 计算参考控制量
v_r = refSpeed(targetIdx);
Delta_r = refDelta(targetIdx);
heading_r = refHeading(targetIdx);

% 实际状态量与参考状态量
X_real = [currentPos(1),currentPos(2),currentHeading];
Xr = [refPos(targetIdx,1), refPos(targetIdx,2), refHeading(targetIdx)];
